...
der = telebot.TeleBot('6330234461:AAENRLIRpqmTDL_X-RqKWslq3PHVO6K1XEM')

@der.message_handler(commands=['start'])
def start(mes):
	d = t.InlineKeyboardButton('قناتي انضم يحلوو', url='t.me/llxxx3')
	de = t.InlineKeyboardMarkup(row_width=2);de.add(d)
	na = mes.from_user.first_name
	acc = der.send_message(mes.chat.id,text=f'<b>Halo {na}, \nthis bot made @llxxx3\n\nEnter username:password</b>', reply_markup=de,parse_mode='HTML')
...
def ax(mes):
		...
		try:
			...
			der.send_message(mes.chat.id,text='عذرا عزيزي 📛\nلا يمكنك الارسال على هذه النحو',reply_markup=de)
			if not (':') in ac:
				return
...
<code>{koe}</code>
---------------------------------------------
@PY_87 - @llxxx3 « انضم</strong>''', parse_mode='html',reply_markup=de)
		else:
			d = t.InlineKeyboardButton('قناتي انضم يحلوو', url='t.me/llxxx3')
			do = t.InlineKeyboardMarkup(row_width=2);do.add(d)
			der.send_message(mes.chat.id,text='عذرا عزيزي 📛\nخطا في تسجيل الدخول الرجاء إعادة المحاولة لطفاً',reply_markup=do)
der.polling()